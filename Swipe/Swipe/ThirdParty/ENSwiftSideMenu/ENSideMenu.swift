...
open class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    */
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffect.Style = .light) {
        ...
        let rightSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: Selector(("handleGesture")))
        rightSwipeGestureRecognizer.delegate = self
        rightSwipeGestureRecognizer.direction =  UISwipeGestureRecognizer.Direction.right
        
        // Add left swipe gesture recognizer
        let leftSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: Selector(("handleGesture")))
        leftSwipeGestureRecognizer.delegate = self
        leftSwipeGestureRecognizer.direction = UISwipeGestureRecognizer.Direction.left
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(ENSideMenu.handleTap2))
        tapGesture.delegate = self
        vwDisable.addGestureRecognizer(tapGesture)
...
*/
    ...
    */
    ...
    fileprivate func setupMenuView() {
        ...
        sideMenuContainerView.layer.shadowOpacity = 0.5
        //sideMenuContainerView.layer.shadowColor = PHColors.NavBarColor.CGColor
        sideMenuContainerView.layer.shadowPath = UIBezierPath(rect: sideMenuContainerView.bounds).cgPath
        sourceView.addSubview(self.sideMenuContainerView)
        ...
        vwDisable.frame = UIScreen.main.bounds
...
//            sideMenuContainerView.addSubview(visualEffectView)
    ...
    @objc internal func handleTap2() {
        self.toggleMenu()
    }
    
    @objc internal func handlePan(_ recognizer : UIPanGestureRecognizer){
        
        _ = recognizer.velocity(in: recognizer.view).x > 0
        ...
        if (isMenuOpen) {
            UIView.animate(withDuration: 0.3, animations: {
                self.blurEffectView.alpha = 0.0
                    ...
                    self.vwDisable.removeFromSuperview()
            })
            toggleMenu(false)
        }
        else {
            ...
            sourceView.addSubview(vwDisable)
            sourceView.insertSubview(blurEffectView, at: sourceView.subviews.count - 1)
