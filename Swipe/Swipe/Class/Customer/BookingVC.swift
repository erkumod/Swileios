...
class BookingCell : UITableViewCell{
    ...
    @IBOutlet weak var ivDot: CustomImageView!
}
...
//            vc.bookingStatus = sender as! String
            ...
            if let washerData = dictData["washer_profile"] as? [String:AnyObject] {
                ...
                if let washer_id = washerData["user_id"] as? String {
                    vc.washerID = "\(washer_id)"
                }
            }
            
            if let unread_message_user = dictData["unread_message_user"] as? [[String:AnyObject]] {
                vc.unreadCnt = unread_message_user.count
            }
...
extension BookingVC : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if isScheduled {
            ...
            cell.lblStatus.textColor = UIColor(red: 131/255, green: 201/255, blue: 99/255, alpha: 1.0)
            
            let price = dictData["fare"] as! String
            cell.lblPrice.text = String(format: "%.2f", Double(price)!)
            ...
            cell.ivDot.backgroundColor = AppColors.cyan
            cell.lblCarType.backgroundColor = AppColors.cyan
            
        }else {
            let dictData = arrHistoryDictData[indexPath.row]
            cell.lblLocation.text = dictData["location"] as? String
            
            cell.ivDot.backgroundColor = UIColor.lightGray
            cell.lblCarType.backgroundColor = UIColor.lightGray
            let brand = dictData["brand_name"] as! String
            let model = dictData["model_name"] as! String
            let plate = dictData["vehicle_no"] as! String
            cell.lblCarDetail.text = "\(brand) \(model) (\(plate))"
            ...
            if let status = dictData["status"] as? String {
                ...
                if status == "Completed" {
                    cell.lblStatus.textColor = UIColor.lightGray//UIColor(red: 131/255, green: 201/255, blue: 99/255, alpha: 1.0)
